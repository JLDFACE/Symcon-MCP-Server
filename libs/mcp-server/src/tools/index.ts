...
?>`;
    ...
    // --- DEV-TOOLS (nur aktiv bei MCP_DEV_TOOLS=1) ---
    symcon_get_script_content: {
      description: 'DEV: Liest den PHP-Inhalt eines bestehenden Skripts (IPS_GetScriptContent). Nutzen, um vorhandene Skripte zu lesen/refactoren, bevor man sie ändert.',
      inputSchema: scriptIdSchema,
      handler: async (args: HandlerArgs) => {
        const { scriptId } = getArgs<z.infer<typeof scriptIdSchema>>(args);
    ...
    symcon_run_script_text_wait: {
      description: 'DEV: Führt PHP-Code aus und WARTET auf das Ende; gibt die echo-Ausgabe zurück (IPS_RunScriptTextWait). Der Dev-Loop: Code ausführen und Ergebnis/Fehler sofort sehen. Achtung: führt beliebiges PHP auf der SymBox aus – nur auf Dev-Systemen.',
      inputSchema: z.object({ scriptText: z.string() }),
      handler: async (args: HandlerArgs) => {
        const { scriptText } = getArgs<{ scriptText: string }>(args);
        const output = await client.runScriptTextWait(scriptText);
