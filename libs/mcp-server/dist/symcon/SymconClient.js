...
export class SymconClient {
    ...
    async runScriptText(scriptText) {
        return this.call('IPS_RunScriptText', [scriptText]);
    }
    /**
     * Führt PHP-Text aus und WARTET auf das Ende (synchron). Gibt die echo-Ausgabe
     * des Skripts als String zurück (IPS_RunScriptTextWait). Ideal für den Dev-Loop:
     * Code ausführen und Ergebnis/Fehler direkt sehen. Nur auf Dev-Systemen freigeben.
     */
    async runScriptTextWait(scriptText) {
        return this.call('IPS_RunScriptTextWait', [scriptText]);
    }
    ...
    static MCP_TIMER_QUEUE_VAR_NAME = 'MCP Timer Queue';
    static MCP_TIMER_DISPATCH_EVENT_NAME = 'MCP Timer Dispatcher';
    /**
